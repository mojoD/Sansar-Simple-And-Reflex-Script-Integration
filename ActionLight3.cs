...
// This script controls an lights color and intensity based on positions of a VR Controller
...
public class ActionLight3 : SceneObjectScript
{
    ...
    // Event, Color, Intensity, Fade Time, BlinkTime, PulseTime, Special 
    private string[] LightEvent = new string[20];
    private Sansar.Color[] LightColor = new Sansar.Color[20];
    private bool[] RandomColor = new bool[20];
    private string[] LightIntensity = new string[20];
    private string[] LightAngle = new string[20];
    private string[] LightFallOff = new string[20];
    private string[] Effect = new string[20];
    private string[] EffectTime = new string[20];
    private string[] EffectParm = new string[20];
    private string[] BaseLightIntensity = new string[20];
    ...
    private string VRControllerHand;
    ...
    private void getVRControllerInfo(ScriptEventData gotVRControllerInfo)
    {
        ...
        SetupVRLight(VRController, VRControllerLocalPosition);
    }

    private void SetupVRLight(string VRControllerIn, Vector VRControllerLocalPosIn)
    {

        if (VRControllerIn == "Right")
        {
            //normalize to about 0.5
            float Red = VRControllerLocalPosIn.X + 0.4f;
            float Green = VRControllerLocalPosIn.Y + 0.12f;
            float Blue = VRControllerLocalPosIn.Z - 1.0f;
            if (Red > 1.0) Red = 1.0f;
            ...
            VRControllerHand = VRControllerIn;
            LightColor[17].R = Red;
            LightColor[17].G = Green;
            LightColor[17].B = Blue;
            LightColor[17].A = 1;
            //Log.Write("New Color: " + LightColor[17]);
        }
    ...
    private void ExecuteLightProgram(ScriptEventData data)
    {
        ...
        if (data.Message == LightEvent[0]) PlayLightProgram(0);
        else if (data.Message == LightEvent[1]) PlayLightProgram(1);
        else if (data.Message == LightEvent[2]) PlayLightProgram(2);
        else if (data.Message == LightEvent[3]) PlayLightProgram(3);
        else if (data.Message == LightEvent[4]) PlayLightProgram(4);
        else if (data.Message == LightEvent[5]) PlayLightProgram(5);
        else if (data.Message == LightEvent[6]) PlayLightProgram(6);
        else if (data.Message == LightEvent[7]) PlayLightProgram(7);
        else if (data.Message == LightEvent[8]) PlayLightProgram(8);
        else if (data.Message == LightEvent[9]) PlayLightProgram(9);
        else if (data.Message == LightEvent[10]) PlayLightProgram(10);
        else if (data.Message == LightEvent[11]) PlayLightProgram(11);
        else if (data.Message == LightEvent[12]) PlayLightProgram(12);
        else if (data.Message == LightEvent[13]) PlayLightProgram(13);
        else if (data.Message == LightEvent[14]) PlayLightProgram(14);
        else if (data.Message == LightEvent[15]) PlayLightProgram(15);
        else if (data.Message == LightEvent[16]) PlayLightProgram(16);
        else if (data.Message == LightEvent[17]) PlayLightProgram(17);
    }
