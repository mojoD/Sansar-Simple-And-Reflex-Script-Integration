...
public class SimpleInteractionWithSecurity : SceneObjectScript
{
    #region EditorProperties
    // The interaction for this object, with a default prompt of Click Me.
    [DefaultValue("Click Me!")]
    [DisplayName("Interaction prompt")]
    public readonly Interaction MyInteraction;

    // The events to send on the first click of this object. Can be a comma separated list of event names.
    ...
    // The events to send on the second click of this object. Can be a comma separated list of event names.
    // The events sent will cycle through only the On Click events with values set.
    [DefaultValue("off")]
    [DisplayName("On 2nd Click ->")]
    public readonly string OnClick2;

    // The events to send on the third click of this object. Can be a comma separated list of event names.
    // The events sent will cycle through only the On Click events with values set.
    [DefaultValue("")]
    [DisplayName("On 3rd Click ->")]
    public readonly string OnClick3;

    // The events to send on the fourth click of this object. Can be a comma separated list of event names.
    // The events sent will cycle through only the On Click events with values set.
    [DefaultValue("")]
    ...
    /*
    // Maximum number of events per second that will trigger events. Set to 0 for no limit.
    [DefaultValue(10)]
    [DisplayName("Max Events Per Second")]
    [Range(0,100)]
    public readonly float MaxEventsPerSecond = 10;

    // Reset this script so the next click will be 1st click.
    ...
    */
    ...
    private void ComplexInteractionHandler()
    {
        MyInteraction.Subscribe((InteractionData idata) =>
        {
            ...
            if (!validUser)
            {
                //MyInteraction.SetPrompt("You Are Not Authorized to Use The Looper");
